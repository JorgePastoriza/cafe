...
const getStats = async (req, res) => {
  try {
    ...
    const [todaySales, monthSales, totalProducts, lowStock] = await Promise.all([
      Sale.findAll({ where: { status: 'completed', created_at: { [Op.between]: [`${today} 00:00:00`, `${today} 23:59:59`] } } }),
      Sale.findAll({ where: { status: 'completed', created_at: { [Op.gte]: monthStart } } }),
      Product.count({ where: { active: true } }),
      Product.count({ where: { active: true, stock: { [Op.lte]: literal('stock_min') } } })
    ]);
  ...
  try {
    ...
    const items = await SaleItem.findAll({
      include: [{
        ...
        where: { status: 'completed', created_at: { [Op.gte]: monthStart } },
        attributes: []
      }],
